...
def sp_cam_vis(model, input_tensor, name, out1):
    ...
    # mask[4]=1
    mask = mask.unsqueeze(0)
    img = vispcam(input_tensor.squeeze(0), cam,
                  mask.unsqueeze(2).unsqueeze(3))*255
...
def camp_vis(map, Qs, input_tensor, mask):
    ...
    map = F.interpolate(map, size=(int(h/16), int(w/16)),
    ...
    map = F.interpolate(map, size=(int(h*2), int(w*2)),
                        mode='bilinear', align_corners=False)

    savepth = 'CAMtest/cam+/'+name

    img = vispcam(input_tensor.squeeze(0), map,
                  mask.unsqueeze(2).unsqueeze(3))*255
    cv2.imwrite(savepth, img)
    # return sp_map
...
weights_dict = models.resnet38_base.convert_mxnet_to_torch(
    './models/ilsvrc-cls_rna-a1_cls1000_ep-0001.params')
weights_dict = torch.load(
    'experiments/log/voc_dyrenum65_thr0.8_25ep/best_checkpoint.pth')
...
Q_model.load_state_dict(torch.load(
    '/media/ders/mazhiming/SP_CAM_code/SPCAM/experiments/models/train_Q_/00.pth'))
...
C, H, W = img.size()
input_tensor = normalize(resize(img, (448, 448)) / 255.,
