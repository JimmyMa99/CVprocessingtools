...
class RandomResize:
    ...
    def __call__(self, image, mode=Image.BICUBIC):
        rand_image_size = random.randint(
...
class RandomResize_For_Segmentation:
    ...
    def __call__(self, data):
        ...
        if size[0] == w and size[1] == h:
            ...
            # cv2.imwrite('1.png',data['cam'].transpose(1,2,0)[:,:,0]*100)
            if ('cam' in data.keys()):
...
class Normalize:
    ...
    def __call__(self, image):
        ...
        norm_image[..., 0] = (image[..., 0] / 255. -
                              self.mean[0]) / self.std[0]
        norm_image[..., 1] = (image[..., 1] / 255. -
                              self.mean[1]) / self.std[1]
        norm_image[..., 2] = (image[..., 2] / 255. -
...
class RandomCrop:
    ...
    def __call__(self, image, bbox_dic=None):
        ...
        cropped_image[dst_bbox['ymin']:dst_bbox['ymax'], dst_bbox['xmin']:dst_bbox['xmax']] = \
            image[src_bbox['ymin']:src_bbox['ymax'],
                  src_bbox['xmin']:src_bbox['xmax']]
...
class RandomCrop_For_Segmentation(RandomCrop):
    ...
    def __call__(self, data):
        ...
        cropped_mask[dst_bbox['ymin']:dst_bbox['ymax'], dst_bbox['xmin']:dst_bbox['xmax']] = \
            mask[src_bbox['ymin']:src_bbox['ymax'],
                 src_bbox['xmin']:src_bbox['xmax']]

        data['image'] = cropped_image
        data['mask'] = cropped_mask
        if ('cam' in data.keys()):
            cropped_cam = np.ones((self.crop_size, self.crop_size,
                                  data['cam'].shape[2]),  data['cam'].dtype) * self.bg_value
            cropped_cam[:, :0] = 1
            cropped_cam[dst_bbox['ymin']:dst_bbox['ymax'], dst_bbox['xmin']:dst_bbox['xmax']] = \
                data['cam'][src_bbox['ymin']:src_bbox['ymax'],
                            src_bbox['xmin']:src_bbox['xmax']]
